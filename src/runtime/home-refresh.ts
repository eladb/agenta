...
export async function refreshHomeMirror(home: HomeConfig, runner: Runner = defaultRunner): Promise<void> {
  let resolved: ReturnType<typeof resolveTransport>;
  try {
    resolved = resolveTransport(home);
  } catch (err) {
    log.warn('home-refresh', `resolveTransport failed for ${home.remote}: ${(err as Error).message}`);
    return;
  }
  ...
  if (!existsSync(join(resolved.mirrorPath, '.git'))) {
    log.warn(
      'home-refresh',
      `mirror at ${resolved.mirrorPath} has no .git; skipping refresh`,
    );
    return;
  }
...
async function refreshHttpsMirror(
  home: HomeConfig,
  mirror: string,
  runner: Runner,
): Promise<void> {
  ...
  if (!token || token.length === 0) {
    log.warn(
      'home-refresh',
      `[${mirror}] auth_env ${home.auth_env} not set; skipping refresh`,
    );
    return;
  }
  let cloneUrl: string;
...
async function refreshDirectMirror(
  ...
  try {
    ...
    try {
      ...
      log.warn(
        'home-refresh',
        `failed to clean up tmp key dir ${dir}: ${(err as Error).message}`,
      );
    }
  }
}

...
describe('github_create_pr', () => {
  describe('describe()', () => {
    ...
    test('shows repo + head_branch when provided', () => {
      expect(
        githubCreatePr.describe?.({ repo: 'eladb/x', head_branch: 'claude/foo' }),
      ).toBe('github PR eladb/x from claude/foo');
    });
  ...
  describe('parseArgs', () => {
    ...
    test('rejects malformed repo', () => {
      expect(() => _internal.parseArgs({ ...ok, repo: 'just-a-name' })).toThrow(/repo.*owner\/name/);

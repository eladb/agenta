...
async function bootstrapDirect(
  ...
  const cloneCmd = [
    ...
    `git clone --depth 1 --recurse-submodules --shallow-submodules ${shellQuote(sshUrl)} agenta-clone &&`,
    `mv agenta-clone/.git ~/ &&`,
    `cp -an agenta-clone/. ~/ &&`,
    `rm -rf agenta-clone`,
    `)`,
...
async function cloneAndCheckout(threadKey: string, repoPath: string): Promise<void> {
  ...
  const cloneCmd = [
    ...
    `cd /tmp &&`,
    `rm -rf agenta-clone &&`,
    `git clone --depth 1 --recurse-submodules --shallow-submodules ${shellQuote(remoteUrl)} agenta-clone &&`,
    `mv agenta-clone/.git ~/ &&`,
    `cp -an agenta-clone/. ~/ &&`,
    `rm -rf agenta-clone`,
    `)`,

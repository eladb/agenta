...
describe('web_search', () => {
  ...
  test('POSTs to Tavily with correct body shape and parses response', async () => {
    ...
    const r = await invokeTool('web_search', JSON.stringify({ query: 'ada lovelace' }), CTX);
  ...
  test('truncates long snippets to ~280 chars', async () => {
    ...
    globalThis.fetch = (async () =>
      new Response(JSON.stringify({ results: [{ title: 't', url: 'https://x', content: big }] }), {
        status: 200,
      })) as unknown as typeof fetch;

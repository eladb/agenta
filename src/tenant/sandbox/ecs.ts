...
async function waitForHealth(ip: string, timeoutMs = 60_000): Promise<void> {
  ...
  throw new Error(
    `ecs sandbox not healthy at ${ip}:${SANDBOX_PORT} after ${timeoutMs}ms: ${String(lastErr)}`,
  );
}
...
async function ensure(threadKey: string): Promise<void> {
  ...
  if (persisted) {
    if (persisted.provider !== 'ecs') {
      ...
      const task = await runTask(threadKey, persisted.sandbox_token, persisted.workspace_path);
  ...
  await waitForHealth(ip);
  log.info('sandbox', `ecs task ${task.taskArn} ready (workspace ${workspacePath}, ip ${ip})`);
}
...
async function killAll(): Promise<void> {
  const tasksResp = await awsJsonTry<ListTasksResp>(['ecs', 'list-tasks', '--cluster', cluster()]);

...
aws ssm put-parameter --type SecureString --name $PREFIX/ACME_SALESFORCE_PLAYGROUND_DEPLOY_KEY --value "$(cat acme-sf-playground-deploy-key.pem)"

# --- Tool credentials ---
# acme_* tools shell out to `acme-cloud` on the bot host.
aws ssm put-parameter --type SecureString --name $PREFIX/ACME_API_TOKEN --value '...'

---
name: canary
description: Run the production smoke test (chat → bash → /delete) against BOTH deploy targets — agenta on Fly and acme on ECS — to confirm the whole system is functional end-to-end. Use when the user asks "is the bot/system working?", "run the canary", "smoke-test prod", "is acme/agenta up?", or after a deploy/cutover when you need a real end-to-end signal (Slack → model → sandbox bash → cleanup), not just a process/health check.
---
...
| **acme** | ECS (cluster + service `agenta-bot`) | `acme` | `U0B65LMHRLL` | `aws ecs describe-services` |

Both bots live in the `agentalabs` workspace and are members of the test channel `C0B307LP274`. They are **different Slack apps** (agenta `A0B2WL8UYAZ`, acme `A0B5VLX7QUT`) → separate event streams → the canary targets each one deterministically by mentioning its user id. There is no split-brain between them.

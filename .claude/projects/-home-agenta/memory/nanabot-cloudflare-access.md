...
**How to apply:** when posting a published-page link to Elad, use the `bensadeh.` host; don't promise a "public URL" without an explicit CF Access *bypass* policy for that path (operator-only host change). Discovered 2026-05-29 building the agenta/acme status page (eladb/agenta #245). Related: [[host-shared-config-rules]].

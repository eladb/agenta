...
**Why:** On 2026-05-31 the acme-staging IAM keys (both `.env` and GH `secrets.AWS_*`) were rotated out mid-fix-forward. The watchdog's acme/ECS canary started failing with `InvalidClientTokenId` on `aws describe-services` — the bot's runtime might have been fine, but no way to tell. The watchdog kept paging on the 30-min cadence. Elad's call: pause the check and stop the firehose.

...
- **`salto` on ECS — deploy extracted to `salto-io/slack-agent` (stand-up in progress).** Core keeps the *generic* ECS capabilities (the `SANDBOX_PROVIDER=ecs` sandbox provider, the Bedrock model gateway, the `AGENTA_ECS_*` env vars, `combo` entrypoint, the generic `scripts/canary.ts` `fly|ecs` branch), but no longer builds or deploys the salto stack — the ECS infra (`infra/ecs/`), deploy scripts (`scripts/deploy-{bot,sandbox}-ecs.ts`), the `salto.json` Slack manifest, and the CD ECS lane were removed (#267, step 2 of the repo split). The salto/ECS *deployment* now lives in the private `salto-io/slack-agent` repo, which consumes agenta as a pinned git submodule, builds an overlay image, and runs its own CD. That repo's stand-up (steps 3–4 of the split) is still pending — until it lands, salto isn't deployed and the canary watchdog's salto check stays paused. Same intended shape as before: a *different* Slack app (`A0B5VLX7QUT`, bot `salto`/`U0B65LMHRLL`) ⇒ separate event streams ⇒ **no split-brain** with the Fly `agenta` bot; ECS on-disk state + exec → the `debug-thread` skill.
- **Canary watchdog** (host-local, not CD). `install.sh` installs a `*/30` cron on the claude-agents host running `~/.local/bin/canary-monitor.sh` → the double canary (agenta/Fly + salto/ECS); on a confirmed red it Slack-alerts `C0B307LP274` and `agents send`s the agenta agent (oncall) to investigate + bounded-remediate (restart only — redeploy/scale/destroy/secret changes escalate). Reusable recipe = the `/canary` skill. The salto/ECS check stays paused until the `salto-io/slack-agent` deployment is stood up, then re-points at it.
...
A **deployment** is `{ bot, N tenants, sandboxes }` all on one cloud. Live today: **agenta-Fly** (bot + `agentalabs` tenant + Fly sandboxes). The **salto-ECS** deployment (bot + `agentalabs` tenant + Fargate sandboxes) is being extracted to the private `salto-io/slack-agent` repo (#267) and isn't stood up yet. Tenant names are deployment-local — same name in two deployments = two separate tenants. The customer is the tenant; the persona (agenta vs. salto) comes from the home repo.
...
For single-tenant deployments (agenta-Fly today; salto-ECS once stood up), bot + tenant run **co-located** in the same container via the `combo` entrypoint role. The split in #253 was correct, but standing up a separate ingress bot app per cloud is operator work that hadn't happened yet; combo collapses both roles into one machine until the cutover, mirroring the dev pattern (`bun run start:bot` + `bun run start:tenant` on one box pointing at loopback). `combo` is generic and stays in core; the salto-ECS combo *deployment config* (task def env + secrets) now lives in `salto-io/slack-agent` (#267).

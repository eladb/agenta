...
src/
  ...
  git/
    ...
    bootstrap.ts           ensureRepoBootstrap(threadKey): generates keypair if needed, writes authorized_keys, drops ssh config + private key + known_hosts into the sandbox volume, shallow-clones AGENTA_REPO_PATH into ~ (over `ssh://agenta-repo`), checks out `agenta/sessions/<thread_key>`. Idempotent. Called from `turn.ts` after `ensureContainer`, before `syncAttachmentsToSandbox`.
  slack/

...
- **Also deployed as `salto` on ECS** — same image, combo entrypoint, on AWS ECS (`SANDBOX_PROVIDER=ecs`, Fargate sandboxes) as a *different* Slack app (`A0B5VLX7QUT`, bot `salto`/`U0B65LMHRLL`). Different app ⇒ separate event streams ⇒ **no split-brain** with the Fly `agenta` bot. ECS on-disk state + exec → the `debug-thread` skill. The salto/ECS **deployment** (infra CFN, deploy scripts, CD lane, Slack manifest) has been **extracted out of OSS core** into the private `salto-io/slack-agent` repo (step 2 of the repo split, #267): that repo consumes agenta as a pinned submodule, builds an overlay image (`AGENTA_EXTRA_TOOLS` + salto-specific tools/home), and deploys to AWS account `870494683302` / `eu-central-1` via the **GitHub OIDC** role `agenta-cd-github` (region-locked, trust-scoped to that repo's main) — **no static AWS keys**. The model is Bedrock (`bedrock://`) via the gateway in core (`src/tenant/model/gateway.ts`). Core keeps the generic ECS sandbox provider + Bedrock gateway but **no longer builds or deploys the salto stack** — core's CD is Fly-only. The new repo is **live** (`salto-io/slack-agent`, cutover 2026-06-01): submodule + overlay image (`agenta/` pinned), OIDC CD **proven green end-to-end** (overlay build → ECR → roll salto ECS → salto canary), and the `agenta-cd-github` trust is now scoped to **`salto-io/slack-agent` only** (eladb/agenta removed at cutover). The one thing **not** yet migrated: the host-local salto/ECS watchdog check stays **paused** in `~/.local/bin/canary-monitor.sh` pending a re-point. (Resource IDs + SSO/OIDC details in the `aws-account-migration` + `repo-split-oss-vs-salto` memories.)
- **Canary watchdog** (host-local, not CD). `install.sh` installs a `*/30` cron on the claude-agents host running `~/.local/bin/canary-monitor.sh` → historically the double canary (agenta/Fly + salto/ECS); on a confirmed red it Slack-alerts `C0B307LP274` and `agents send`s the agenta agent (oncall) to investigate + bounded-remediate (restart only — redeploy/scale/destroy/secret changes escalate). Reusable recipe = the `/canary` skill. **Both halves are now effectively idle**: the agenta/Fly target was torn down (#276, no app to canary), and the salto/ECS half is **paused** — salto deploys from `salto-io/slack-agent` (its own CD canaries salto on each deploy); re-pointing this host watchdog's salto check at that deployment is the remaining follow-up.
...
- **`bun run deploy` / `bun run canary` no longer have a live target** — the agenta Fly apps were torn down 2026-06-02 (#276). The scripts (`deploy-bot-fly.ts`, `canary.ts`) and the org-scoped `FLY_API_TOKEN` in `.env` remain for reference, but running them would recreate the prod app — don't, unless an operator explicitly wants agenta Fly back. CD no longer invokes either.
- **CD is test-only** (`.github/workflows/cd.yml`, #276): pushing to `main` runs the e2e suite on the **docker** sandbox provider (against `agenta-ci` so prod Socket Mode is untouched) and stops — no deploy, no canary (the Fly target is gone). The docker provider also exercises the ~13 docker-gated sandbox test cases (`test.if(DOCKER_PROVIDER_ACTIVE)`) that the old fly-provider run skipped — a coverage gain, but it means the first docker-e2e CD run is the shakedown. The salto/ECS lane was extracted to `salto-io/slack-agent` (#267) — core's CD never touched AWS (no OIDC, no `id-token` permission). **Gating is asymmetric** — branch protection requires only `unit-tests`; e2e runs post-merge only, so e2e regressions can land red. Two skip tiers remain (`.ci-ignore` doc-only, `.ci-e2e-ignore` prod-only paths like `config/tenants.json`, `slack-manifests/*`); with deploy/canary gone a prod-only commit now no-ops entirely, but the tiers are left in place (reworking change-detection is out of scope here).
...
The `combo` entrypoint itself is **generic** and stays in core (used by agenta-Fly). The salto ECS combo *deployment config* (CFN task-def env, `command: [combo]`, the OIDC CD lane) was extracted to the private `salto-io/slack-agent` repo (#267); core no longer ships salto's ECS infra.
...
**CD secrets (GH Actions, not `.env`):** `CI_SLACK_APP_TOKEN`, `CI_SLACK_BOT_TOKEN`, `TEST_APP_TOKEN`, `TEST_BOT_TOKEN`, `MODEL_API_KEY`. The old deploy/canary secrets (`FLY_API_TOKEN`, `FLY_SANDBOX_API_TOKEN`, `SLACK_BOT_TOKEN`, `CANARY_CHANNEL_ID`) are now unused by CD (#276 made it test-only) — still set in the repo, harmless; deleting them is an operator task.

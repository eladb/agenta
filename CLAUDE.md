...
The `combo` entrypoint itself is **generic** and stays in core (used by agenta-Fly). The acme ECS combo *deployment config* (CFN task-def env, `command: [combo]`, the OIDC CD lane) was extracted to the private `acme-io/slack-agent` repo (#267); core no longer ships acme's ECS infra.
...
- **Acme** A0B5VLX7QUT (bot `U0B65LMHRLL`, `acme`) — same codebase deployed on **ECS** (not Fly), Bedrock model, its own home config. Distinct app ⇒ no Socket Mode split-brain with `agenta`.

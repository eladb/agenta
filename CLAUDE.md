...
23. **Fly transport for the git bootstrap (reverse SSH tunnel)** — phase 22's bootstrap reached the Mac's sshd via `host.docker.internal:22` which is a Docker-Desktop-only loopback alias; on Fly there's no equivalent route from a Firecracker VM behind 6PN. Phase 23 added a per-sandbox **autossh reverse tunnel over Fly's WireGuard mesh** (dropbear in the image listening on 2200, autossh on the Mac, `~/.ssh/agenta_tunnel_id_ed25519` keypair, WireGuard peer setup via `bun run setup-tunnel`). **Replaced by phase 24** — kept here only for historical context.
24. **WebSocket tunnel for git transport** — phases 22 + 23's per-session SSH transport (Mac sshd + authorized_keys lines + dropbear-in-the-image + autossh + WireGuard) was operationally and cognitively expensive. Phase 24 replaces all of it with a single per-session **WebSocket tunnel over the existing sandbox HTTP API**. Direction is inverted: the bot opens the WS (it already reaches the sandbox over Bearer-auth'd HTTP — same path as `/exec`, `/write`, etc.), and the WS multiplexes loopback TCP between the sandbox's git client and the bot's local git HTTP server. Pieces:
    - `sandbox/server/server.ts:/tunnel` — new WS route on the existing in-container Bun HTTP server. On open: binds a TCP listener on `127.0.0.1:6000` inside the sandbox; per `accept()` it allocates a u32 streamId and frames bytes both ways. Wire format is a 5-byte header (`streamId u32 BE | type u8`) + payload, type 0 = data, type 1 = close.
    - `src/git/git-server.ts:startGitServer({ repoPath, allowedRef, hooksDir })` — bot-side per-session HTTP server on `127.0.0.1:0`. Wraps `git http-backend` as a CGI subprocess per request. Sets `core.hooksPath` + `AGENTA_ALLOWED_REF` via `GIT_CONFIG_COUNT`/`GIT_CONFIG_KEY_0`/`GIT_CONFIG_VALUE_0` env so the agenta pre-receive policy runs without touching the customer repo's hook tree. Accepts `/<repo>.git/(info/refs|git-upload-pack|git-receive-pack)` and rewrites `.git` off PATH_INFO before spawning (http-backend wants the suffix-less form for a non-bare repo).
    - `src/git/ws-tunnel.ts:startTunnel({ threadKey, sandboxBaseUrl, sandboxToken, localTargetPort })` — bot-side WS client. Opens `<base>/tunnel` (scheme rewritten http→ws / https→wss) with `Authorization: Bearer <token>`. Per inbound data frame with a new streamId: opens a TCP socket to `127.0.0.1:<localTargetPort>` (the bot's git server). Pipes bytes both ways; CLOSE frame in either direction tears down its TCP socket. Reconnects with backoff up to 5s on WS drop; cleared on `stop()`.
    - `src/git/bootstrap.ts:ensureRepoBootstrap(threadKey)` — orchestrator. Fast-path: if the per-thread handle is alive AND `session.git.ref` matches AND `test -d ~/.git` in the sandbox succeeds, returns. Otherwise: start the git server → start the tunnel → probe `bash -c 'exec 3<>/dev/tcp/localhost/6000'` (caps at 5s) → persist `session.git = { ref }` → clone into the sandbox via `http://localhost:6000/<repo>.git` → `git -C ~ remote set-url origin …` (idempotent, in case the random gitServer port changed since last bootstrap) → configure identity → `checkout -B agenta/sessions/<thread_key>`. Per-thread `Map<threadKey, {gitServer, tunnel}>` survives across `runTurn` invocations. `teardownSession(threadKey)` stops both halves; called from `/delete` (before `removeContainer`) and from `process.on('exit'|'SIGINT'|'SIGTERM')`.
    - **Removed**: `bin/agenta-git-shell`, `src/git/{keys,authorized-keys,known-hosts,tunnel}.ts`, `scripts/setup-tunnel.ts`, `dropbear-bin` from the sandbox image, all dropbear setup in `entrypoint.sh`, the Mac sshd / WireGuard prerequisites, the `AGENTA_HOST_SSH_HOSTNAME`/`AGENTA_HOST_SSH_PORT`/`AGENTA_AUTHORIZED_KEYS_PATH` env vars, the `reapOrphanAuthorizedKeys` boot-time sweep, the per-session SSH keypair generation. `git-hooks/pre-receive` stays unchanged — same policy, now wired in by `git-server.ts` instead of `agenta-git-shell`.
    - **`session.json.git` shape**: `{ ref }` going forward. The `pubkey_fp` field is tolerated on read for backwards-compat with sessions written under phase 22/23, but no longer populated.
    - **E2E**: `tests/e2e/git-botspace.test.ts` is rewritten around the WS transport — same docker round-trip (`git clone` + commit + push back), no opt-in environment variables, no real `~/.ssh/authorized_keys` mutation. Gated solely on `DOCKER_PROVIDER_ACTIVE`. The Fly path is identical at the transport layer — the WS tunnel doesn't care which provider is underneath — so the docker test covers it.

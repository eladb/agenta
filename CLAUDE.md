...
- `BOTSPACE_READ_TOKEN` — fine-grained PAT with read access to `eladb/agenta-test-home`; used to clone the botspace into `$RUNNER_TEMP` so the in-process agent's `buildSystemPrompt()` can resolve `AGENTA_REPO_PATH`

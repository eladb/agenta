...
sandbox/
  Dockerfile               multi-stage: oven/bun:1-slim builds the server binary → ubuntu:24.04 runtime + iptables/ripgrep/git/curl/jq/python3/python3-pil/matplotlib/numpy/pandas/imagemagick, sandbox user uid 1000. `COPY --chown=sandbox:sandbox botspace /opt/botspace/` — the seed lives at /opt/botspace/ because /home/sandbox is overlaid by the per-thread volume mount at run time. `WORKDIR /home/sandbox`.
  botspace/                README.md + skills/<slug>/SKILL.md library. Single source of truth for the bot's prompt; baked into /opt/botspace/ in the image and copy-if-missing'd into a fresh volume at /home/sandbox by entrypoint.sh on first boot. Override the dir for tests via `BOTSPACE_DIR` env.
  entrypoint.sh            installs iptables OUTPUT rules (root + NET_ADMIN), seeds a fresh /home/sandbox volume from /opt/botspace (copy-if-missing, keyed on README.md), then `setpriv` to sandbox user with bounding/inheritable/ambient caps wiped, then exec the server

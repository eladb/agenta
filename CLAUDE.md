...
- **Local `bun run deploy` fails** with the app-scoped `FLY_API_TOKEN` in `.env` (needs broader scope). Workaround: `flyctl deploy -a agenta-bot --remote-only` from your shell. CD has the right scope via its own secret.

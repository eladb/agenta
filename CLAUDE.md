...
```
src/
  index.ts                 entry: env → acquire('agent') lock → connect → listen (registered FIRST) → fire-and-forget reapOrphanSandboxes + recoverInterruptedSessions
...
sandbox/
  Dockerfile               multi-stage: oven/bun:1-slim builds the server binary → ubuntu:24.04 runtime + iptables/ripgrep/git/curl/jq/zip/unzip + python3/python3-pip/python3-pil/matplotlib/numpy/pandas/imagemagick, sandbox user uid 1000. `COPY --chown=sandbox:sandbox botspace /opt/botspace/` — the seed lives at /opt/botspace/ because /home/sandbox is overlaid by the per-thread volume mount at run time. `WORKDIR /home/sandbox`. Note: Ubuntu 24.04 enforces PEP 668 so `pip install` outside a venv needs `--break-system-packages`.
  botspace/                README.md + skills/<slug>/SKILL.md library. Single source of truth for the bot's prompt; baked into /opt/botspace/ in the image and copy-if-missing'd into a fresh volume at /home/sandbox by entrypoint.sh on first boot. Override the dir for tests via `BOTSPACE_DIR` env. README.md includes Narration rules (one short sentence before each tool call), File-handling rules (strict share_file semantics), and the standard-markdown-only instruction.
  entrypoint.sh            chown /home/sandbox to sandbox:sandbox (Fly mounts fresh volumes as root:root mode 0755), then seeds the volume from /opt/botspace (copy-if-missing, keyed on ~/README.md); conditionally installs the iptables OUTPUT block ONLY when `SANDBOX_EGRESS=block` (default: allow); then `setpriv` to sandbox user with bounding/inheritable/ambient caps wiped, then exec the server.

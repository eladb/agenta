...
```
src/
  index.ts                 entry: env → connect → reapOrphanSandboxes → recoverInterruptedSessions → listen (per-thread sandboxes survive restarts; reap is a safety net for orphans with no matching session.json)
  ...
  runtime/
    ...
    session-store.ts       atomic temp+rename session.json per thread; schema {status, updated_at, system_prompt?, sandbox?}; clearSession now rewrites idle (preserving system_prompt + sandbox), only /delete removes the file. setSandbox(threadKey, record|undefined) is the atomic helper providers call.
  ...
  sandbox/
    ...
    docker.ts              dockerProvider — local Docker container. ensureImage / ensureNetwork / dockerSpawn + lifecycle. ensure re-hydrates from session.json (verifyAlive via `docker inspect`) before provisioning; getEndpoint also lazily re-hydrates; remove + killAll sweep the disk-side sandbox field.
    fly.ts                 flyProvider — per-thread Fly machine via the Fly Machines REST API. fly-force-instance-id header routes requests to the specific machine over the shared <app>.fly.dev URL. Same re-hydration shape as docker.ts (verifyAlive via `GET /v1/apps/<app>/machines/<id>` and `state === 'started'`).
    index.ts               provider selector (SANDBOX_PROVIDER=docker|fly), provider-agnostic HTTP client: runBash/readFile/readBinary/writeFile/editFile/grep/glob/listDir, consumeExecStream (SSE parser). Re-exports containerName for tests. Hosts reapOrphanSandboxes() (boot-time safety net).
sandbox/
...
tests/e2e/
  helpers.ts               startAgent (in-process), startTester, mention, uploadFile, waitForReply, waitFor, deleteThread (also removeContainer), stubCalls/recordingStub, createGoldenCallModel
  fixtures.ts              inline PNG/PDF/text/binary byte fixtures
  *.test.ts                echo, commands, persistence, edits, attachments, session, tools, sandbox, skills, skills-golden
tests/golden/

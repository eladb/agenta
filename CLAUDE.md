...
- **Agent home lives on GitHub** at `eladb/agenta-test-home` (private). `entrypoint.sh` clones it into `/data/agent-home` (= `AGENT_HOME`) on first boot using `GITHUB_TOKEN`; on subsequent boots it `fetch + reset --hard origin/main` so README/skill edits made on GitHub reach the bot. `git-hooks/post-receive` auto-pushes every `agenta/sessions/<thread_key>` ref to `origin` after the WS-tunnel receive-pack completes — best-effort, failures log to stderr and don't block the receive.
...
src/
  ...
  prompt.ts                buildSystemPrompt(agentHomeDir?, envPrefix?): walks the configured host-side directory (default `AGENT_HOME`; `AGENT_HOME_DIR` override for tests) to compose [env prefix] + README.md + "Available skills" + JSON array (sorted by path). Throws when neither env is set. Pure, no Slack/sandbox deps. Skills with bad frontmatter are warn-and-skipped.
  git/
...
- `AGENT_HOME` — absolute path to a non-bare git working tree on the bot host. It's both the source of the prompt (README.md + skills/) AND the remote the per-session sandbox clones from + pushes back to. Required for `bun start`; only optional in tests (set `AGENT_HOME_DIR` instead to point the prompt builder at a tmpdir). On Fly, `entrypoint.sh` clones from `AGENT_HOME_REPO` into this path on first boot.
- `AGENT_HOME_REPO` — `owner/repo` of the GitHub repo `entrypoint.sh` clones into `AGENT_HOME` on Fly. Set in `fly.toml`'s `[env]` (default `eladb/agenta-test-home`); on subsequent boots `entrypoint.sh` fast-forwards `main` from origin. Ignored when the path doesn't match a Fly-style mount, so it's a no-op for local `bun start` against a checked-out repo.
- `GITHUB_TOKEN` — fine-grained PAT with read+write to `AGENT_HOME_REPO`. Used by `entrypoint.sh` to clone on first boot and embedded in the `origin` URL so the `post-receive` hook can auto-push session refs. Fly secret only; not needed for local `bun start`.
...
- `SYSTEM_PROMPT` — **prepended** to README.md (separated by a blank line). Used to **replace** the default const prompt entirely; the switch to a per-thread frozen prompt composed from `<AGENT_HOME>/README.md` made replace-semantics meaningless, so it's now a prefix. Leave unset to use README.md verbatim.
- `AGENT_HOME_DIR` — override the directory the prompt is composed from (precedence: `AGENT_HOME_DIR` > `AGENT_HOME`). E2E + unit tests use this to point at an isolated tmpdir so they don't need a real repo on disk.
...
- `AGENT_HOME_READ_TOKEN` — fine-grained PAT with read access to `eladb/agenta-test-home`; used to clone the agent home into `$RUNNER_TEMP` so the in-process agent's `buildSystemPrompt()` can resolve `AGENT_HOME`
...
- **Auto-merge is the default**. The `issue-driven` skill calls `gh pr merge <N> --auto --squash --delete-branch` right after `gh pr create`, so PRs land themselves the moment CI is green. The user can override by saying "let me review first" on a specific PR.
